                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const newSpecialties = specialties.filter(s => norm(s) !== norm(specialty));
                      console.log('❌ Removing specialty:', specialty, '→ New list:', newSpecialties);
        ...
        {!sportsLoading && !sportsError && databaseSports.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-80 overflow-y-auto">
            {databaseSports
              ...
              .map((sport) => {
                // Normalização segura e robusta
                const norm = (s) => s.trim().toLowerCase();
                const safeSpecialties = Array.isArray(specialties) ? specialties : [];
                const isSelected = safeSpecialties.some(s => norm(s) === norm(sport.name));
                const sportIcon = getSportIcon(sport.name);
                
                const handleToggle = () => {
                  console.log('🔄 Toggle sport:', sport.name, 'Currently selected:', isSelected);
                  console.log('📝 Current specialties:', safeSpecialties);
                  
                  if (isSelected) {
                    // Remove da lista usando normalização
                    const newSpecialties = safeSpecialties.filter(s => norm(s) !== norm(sport.name));
                    ...
                    if (safeSpecialties.length < 8) {
                      const newSpecialties = [...safeSpecialties, sport.name];
                ...
                return (
                  <button
                    ...
                    disabled={!isSelected && safeSpecialties.length >= 8}
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium text-center transition-colors duration-200 leading-tight pointer-events-none ${
                      isSelected
